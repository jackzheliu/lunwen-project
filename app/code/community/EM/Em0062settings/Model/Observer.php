...
class EM_Em0062settings_Model_Observer {
	...
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		...
		if (Mage::helper('em0062settings')->getGeneral_DisableFrontendVariation()) {
		...
		if (Mage::helper('em0062settings')->getGeneral_DisableFooterLinks()) {
	...
	public function beforeCatalogProductCollectionLoad(Varien_Event_Observer $observer) {
		$alt_img = Mage::helper('em0062settings')->getProductsGrid_AltImg();
		if ($alt_img == 'image')
